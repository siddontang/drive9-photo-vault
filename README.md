...
- Video jobs are fenced by photo id, source object/checksum, share token, and rendition version. Revoked, archived, deleted, or replaced sources cannot publish stale output. Retryable failures remove partial outputs and remain `preparing`; only a deterministic privacy failure or an exhausted retry budget exposes terminal `503`, with exhausted jobs sent to the dead-letter queue.
