import { useEffect, useState } from 'react';

export const COPY = {
  en: {
    tagline: 'Upload. Search. Organize. Photos & Videos.',
    api: 'API',
    choose: 'Choose files',
    working: 'Working…',
    addOptions: 'Add tags',
    hideOptions: 'Hide options',
    placeholderTags: 'Tags, comma separated',
    placeholderAlbum: 'Album',
    placeholderNote: 'Note',
    searchPlaceholder: 'Search photos & videos…',
    refresh: 'Refresh',
    all: 'All',
    statsPhotos: 'photos',
    statsFavorites: 'favorites',
    summary: 'Summary',
    showMore: 'show more',
    showLess: 'show less',
    tagsLabel: 'Tags',
    hide: 'hide',
    moreSuffix: (n) => `+${n} more`,
    delete: 'Delete',
    editTags: 'Edit tags',
    addTag: 'Add',
    manageTags: 'Delete',
    done: 'Done',
    removeTag: (tag) => `Remove ${tag}`,
    saveTags: 'Save',
    cancel: 'Cancel',
    addTagPlaceholder: 'New tag',
    tagEditorPlaceholder: 'Add / edit tags, comma separated',
    noTags: 'No tags yet',
    statsVideos: 'videos',
    pending: 'drive9 analyzing… refresh/search again in a few seconds',
    pendingVideo: 'drive9 analyzing video… this may take a while',
    pendingTimeout: 'Analysis has not returned yet. It may still be processing, or may not be available for this item. Try refreshing later.',
    analysisUnavailable: 'Analysis unavailable for this item (it may exceed the current analysis size limit).',
    empty: 'No media yet. Tap "Choose files" to start.',
    poweredBy: 'Powered by',
    coachMark: 'Tap to switch · 切换语言',
    details: 'Details',
    close: 'Close',
    previous: 'Previous photo',
    next: 'Next photo',
    info: 'Info',
    photoCounter: (idx, total) => `${idx} / ${total}`,
    noMetadata: 'No additional details',
    share: 'Share',
    manageShare: 'Manage share',
    shareNow: 'Share…',
    shared: 'Shared',
    shareTo: 'Share to',
    shareX: 'Share on X',
    shareFacebook: 'Share on Facebook',
    shareWechat: 'Share with WeChat',
    x: 'X',
    facebook: 'Facebook',
    wechat: 'WeChat',
    copy: 'Copy',
    more: 'More',
    wechatQrTitle: 'Scan with WeChat',
    wechatQrBody: 'This QR code is generated privately on this device.',
    copyShareLink: 'Copy link',
    copied: 'Copied',
    shareAccessTitle: 'Anyone with the link',
    shareAccessBody: 'Can view this item, but not your library.',
    sharePreparingTitle: 'Preparing a privacy-safe copy…',
    sharePreparingBody: 'The first share prepares a privacy-safe copy. You can close this panel; processing will continue.',
    sharePreparingPublic: 'This privacy-safe video is still preparing…',
    sharePreparationTimeout: 'This privacy-safe video is taking longer than expected. Refresh or try again later.',
    sharePreparationFailed: 'The privacy-safe video could not be prepared. Ask the owner to retry sharing it.',
    shareLinkCreated: 'Share link created; privacy-safe video is still preparing',
    shareReady: 'Share link is ready',
    manualCopyHelp: 'Copy the link below',
    revokeLink: 'Revoke link',
    revokeConfirmTitle: 'Stop sharing?',
    revokeConfirmBody: 'The current link will stop working immediately.',
    keepSharing: 'Keep sharing',
    revokeConfirm: 'Revoke',
    shareCopied: 'Share link copied',
    shareRevoked: 'Share link revoked',
    shareFailed: (msg) => `Could not update share link: ${msg}`,
    sharedPhoto: 'Shared photo',
    loadingShare: 'Loading shared photo…',
    shareUnavailable: 'This share link is unavailable or has been revoked.',
    progressPreparing: (n) => `Preparing ${n} file${n > 1 ? 's' : ''}…`,
    progressUploading: (name, idx, total) => `Uploading ${name} (${idx}/${total})…`,
    progressIndexing: (idx, total) => `Saving to drive9 and extracting metadata (${idx}/${total})…`,
    progressIndexed: (name, idx, total) => `Indexed ${name} (${idx}/${total})…`,
    progressPending: (name) => `Uploaded ${name}. drive9 is still analyzing; tags will appear after refresh.`,
    progressRefreshing: 'Refreshing library…',
    progressDone: 'Done',
    errorLoad: (msg) => `Could not load photos: ${msg}`,
  },
  zh: {
    tagline: '上传 · 搜索 · 整理 照片与视频',
    api: '接口',
    choose: '选择文件',
    working: '处理中…',
    addOptions: '添加标签',
    hideOptions: '收起选项',
    placeholderTags: '标签，用逗号分隔',
    placeholderAlbum: '相册',
    placeholderNote: '备注',
    searchPlaceholder: '搜索照片和视频…',
    refresh: '刷新',
    all: '全部',
    statsPhotos: '张照片',
    statsFavorites: '收藏',
    summary: '摘要',
    showMore: '展开',
    showLess: '收起',
    tagsLabel: '标签',
    hide: '收起',
    moreSuffix: (n) => `还有 ${n} 项`,
    delete: '删除',
    editTags: '编辑标签',
    addTag: '添加',
    manageTags: '删除',
    done: '完成',
    removeTag: (tag) => `删除 ${tag}`,
    saveTags: '保存',
    cancel: '取消',
    addTagPlaceholder: '新标签',
    tagEditorPlaceholder: '添加 / 修改标签，用逗号分隔',
    noTags: '暂无标签',
    statsVideos: '个视频',
    pending: 'drive9 正在分析… 几秒后刷新或再次搜索即可看到结果',
    pendingVideo: 'drive9 正在分析视频… 可能需要较长时间',
    pendingTimeout: '分析尚未返回，可能仍在处理中，或当前不支持此文件。请稍后刷新重试。',
    analysisUnavailable: '该文件暂无分析结果（可能超过当前分析大小上限）。',
    empty: '还没有内容，点击 "选择文件" 开始。',
    poweredBy: '由',
    coachMark: '切换语言 · Tap to switch',
    details: '详情',
    close: '关闭',
    previous: '上一张',
    next: '下一张',
    info: '详情',
    photoCounter: (idx, total) => `${idx} / ${total}`,
    noMetadata: '暂无更多信息',
    share: '分享',
    manageShare: '管理分享',
    shareNow: '分享…',
    shared: '已分享',
    shareTo: '分享到',
    shareX: '分享到 X',
    shareFacebook: '分享到 Facebook',
    shareWechat: '通过微信分享',
    x: 'X',
    facebook: 'Facebook',
    wechat: '微信',
    copy: '复制',
    more: '更多',
    wechatQrTitle: '微信扫码分享',
    wechatQrBody: '二维码只在当前设备本地生成，不会发送到第三方服务。',
    copyShareLink: '复制链接',
    copied: '已复制',
    shareAccessTitle: '拿到链接的人',
    shareAccessBody: '可以查看这一项，但不能访问你的图库。',
    sharePreparingTitle: '正在制作隐私安全副本…',
    sharePreparingBody: '首次分享需要制作隐私安全副本。你可以先收起这个面板，后台会继续处理。',
    sharePreparingPublic: '隐私安全视频仍在准备中…',
    sharePreparationTimeout: '隐私安全视频准备时间较长，请刷新页面或稍后再试。',
    sharePreparationFailed: '隐私安全视频准备失败，请让分享者重新尝试。',
    shareLinkCreated: '分享链接已创建，隐私安全视频仍在准备中',
    shareReady: '分享链接已准备好',
    manualCopyHelp: '请复制下方链接',
    revokeLink: '停止分享',
    revokeConfirmTitle: '停止分享？',
    revokeConfirmBody: '当前链接将立即失效。',
    keepSharing: '继续分享',
    revokeConfirm: '停止分享',
    shareCopied: '分享链接已复制',
    shareRevoked: '分享链接已失效',
    shareFailed: (msg) => `更新分享链接失败：${msg}`,
    sharedPhoto: '分享的照片',
    loadingShare: '正在加载分享内容…',
    shareUnavailable: '此分享链接不可用或已被撤销。',
    progressPreparing: (n) => `正在准备 ${n} 个文件…`,
    progressUploading: (name, idx, total) => `正在上传 ${name}（${idx}/${total}）…`,
    progressIndexing: (idx, total) => `正在写入 drive9 并提取媒体信息（${idx}/${total}）…`,
    progressIndexed: (name, idx, total) => `已索引 ${name}（${idx}/${total}）…`,
    progressPending: (name) => `已上传 ${name}。drive9 仍在分析，刷新后会出现标签。`,
    progressRefreshing: '正在刷新照片库…',
    progressDone: '完成',
    errorLoad: (msg) => `加载照片失败：${msg}`,
  },
};

function detectInitialLang() {
  const saved = typeof localStorage !== 'undefined' && localStorage.photoVaultLang;
  if (saved === 'zh' || saved === 'en') return saved;
  if (typeof navigator !== 'undefined' && navigator.language?.toLowerCase().startsWith('zh')) return 'zh';
  return 'en';
}

export function useLang() {
  const [lang, setLang] = useState(detectInitialLang);

  useEffect(() => {
    localStorage.photoVaultLang = lang;
    document.documentElement.lang = lang === 'zh' ? 'zh-Hans' : 'en';
  }, [lang]);

  return { lang, setLang, t: COPY[lang] };
}

export function fmtBytes(n = 0) {
  if (n > 1024 * 1024) return `${(n / 1024 / 1024).toFixed(1)} MB`;
  if (n > 1024) return `${(n / 1024).toFixed(0)} KB`;
  return `${n} B`;
}

export function fmtDate(value, lang = 'en') {
  if (!value) return '';
  const d = new Date(value);
  if (Number.isNaN(d.getTime())) return '';
  const locale = lang === 'zh' ? 'zh-CN' : 'en-US';
  return new Intl.DateTimeFormat(locale, { year: 'numeric', month: 'short', day: 'numeric' }).format(d);
}

export function pickLangField(photo, lang, key) {
  const primary = photo[`${key}${lang === 'zh' ? 'Zh' : 'En'}`];
  const fallback = photo[`${key}${lang === 'zh' ? 'En' : 'Zh'}`];
  return primary || fallback || '';
}

export function pickLangTags(photo, lang) {
  const manual = Array.isArray(photo.tags) ? photo.tags.filter(Boolean) : [];
  if (manual.length) return [...new Set(manual)];
  const primary = lang === 'zh' ? photo.aiTagsZh : photo.aiTagsEn;
  const fallback = lang === 'zh' ? photo.aiTagsEn : photo.aiTagsZh;
  if (primary && primary.length) return [...new Set(primary.filter(Boolean))];
  if (fallback && fallback.length) return [...new Set(fallback.filter(Boolean))];
  return [];
}
